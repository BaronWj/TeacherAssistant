...
- (void)viewDidLoad
{
    ...
    if(IsAfterIOS7){
        ...
        _chooseSchool_tableView.sectionIndexColor = [UIColor whiteColor];
    }else{
        ...
        if ([arr count] > indexPath.row) {
         ...
         NSString * icon_url =[self searchData:chineseStr.string];
           if (!ISNULLSTR(icon_url)) {
              [_SchoolICon setImageWithURL:[NSURL URLWithString:icon_url]placeholderImage:[UIImage imageNamed:@"teacherLogin"]];
           }else{
              [_SchoolICon setImageWithURL:nil placeholderImage:[UIImage imageNamed:@"teacherLogin"]];
           }
        }
    }
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    myView.backgroundColor = UIColorFromRGB(0x21555c);
