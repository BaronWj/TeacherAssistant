...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.numRoom.textColor = [UIColor whiteColor];
        self.numRoom.hidden = YES;
        self.numRoom.center=CGPointMake(self.frame.size.width*0.5, self.frame.size.height*0.25);
        [self addSubview:self.numRoom];
        
        self.detailRoom=[[UILabel alloc]initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
        self.detailRoom.textAlignment=NSTextAlignmentCenter;
        self.detailRoom.font=[UIFont systemFontOfSize:12];
        self.detailRoom.textColor = [UIColor whiteColor];
//        self.detailRoom.center=CGPointMake(self.frame.size.width, self.frame.size.height-self.frame.size.height);
